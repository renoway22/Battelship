 ...
 */
public class FluffyUnicornsOfDoom extends ComputerOpponent {

    /**
     * This is this class' constructor.
     * @param name The player's name.
     */
    public FluffyUnicornsOfDoom(String name) {
    ...
    public void huntingMode (Coordinates coordinates) {
    	...
    	if ((lastTargetHit.get(i).x==lastTargetHit.get(i-1).x) && (lastTargetHit.get(i).y<lastTargetHit.get(i-1).y)) {
            ...
            Coordinates downRight = new Coordinates (lastTargetHit.get(i-1).x+1, lastTargetHit.get(i-1).y);
            Coordinates downLeft = new Coordinates (lastTargetHit.get(i-1).x-1, lastTargetHit.get(i-1).y);
            if (0<=up.x && 0<=up.y) {
    			if (up.x <10 && up.y <10) 
            ...
            if (0<=downRight.x && 0<=downRight.y) {
    			if (downRight.x <10 && downRight.y <10) {
            rejectedCoordinates.add(downRight);
    			}
            }
            if (0<=downLeft.x && 0<=downLeft.y) {
    			if (downLeft.x <10 && downLeft.y <10) {
            rejectedCoordinates.add(downLeft);
    			}
            }
       	}
        ...
        if ((lastTargetHit.get(i).x==lastTargetHit.get(i-1).x) && (lastTargetHit.get(i).y>lastTargetHit.get(i-1).y)) {
               ...
               possibleTargets = new ArrayList<Coordinates>();
                ...
                Coordinates upRightT = new Coordinates (lastTargetHit.get(i).x+1, lastTargetHit.get(i).y);
                Coordinates upLeftT = new Coordinates (lastTargetHit.get(i).x-1, lastTargetHit.get(i).y);
                Coordinates downRightT = new Coordinates (lastTargetHit.get(i-1).x+1, lastTargetHit.get(i-1).y);
                Coordinates downLeftT = new Coordinates (lastTargetHit.get(i-1).x-1, lastTargetHit.get(i-1).y);
                if (0<=upT.x && 0<=upT.y) {
                ...
                if (0<=upRightT.x && 0<=upRightT.y) {
        			if (upRightT.x <10 && upRightT.y <10) {
                rejectedCoordinates.add(upRightT);
        			}
                }
                if (0<=upLeftT.x && 0<=upLeftT.y) {
        			if (upLeftT.x <10 && upLeftT.y <10) {
                rejectedCoordinates.add(upLeftT);
        			}
                }
                if (0<=downRightT.x && 0<=downRightT.y) {
        			if (downRightT.x <10 && downRightT.y <10) {
                rejectedCoordinates.add(downRightT);
        			}
                }
                if (0<=downLeftT.x && 0<=downLeftT.y) {
        			if (downLeftT.x <10 && downLeftT.y <10) {
                rejectedCoordinates.add(downLeftT);
        			}
                }
		...
		if ((lastTargetHit.get(i).y==lastTargetHit.get(i-1).y) && (lastTargetHit.get(i).x>lastTargetHit.get(i-1).x)) {
            ...
            Coordinates rightUp = new Coordinates (lastTargetHit.get(i).x, lastTargetHit.get(i).y-1);
            Coordinates rightDown = new Coordinates (lastTargetHit.get(i).x, lastTargetHit.get(i).y+1);
            Coordinates leftUp = new Coordinates (lastTargetHit.get(i-1).x, lastTargetHit.get(i-1).y-1);
            Coordinates leftDown = new Coordinates (lastTargetHit.get(i-1).x, lastTargetHit.get(i-1).y+1);
            if (0<=right.x && 0<=right.y) {
    			if (right.x <10 && right.y <10)
            possibleTargets.add(right);
            }
            if (0<=rightUp.x && 0<=rightUp.y) {
    			if (rightUp.x <10 && rightUp.y <10)
    				rejectedCoordinates.add(rightUp);
            }
            if (0<=rightDown.x && 0<=rightDown.y) {
    			if (rightDown.x <10 && rightDown.y <10)
    				rejectedCoordinates.add(rightDown);
            }
            if (0<=left.x && 0<=left.y) {
    			if (left.x <10 && left.y <10)
            possibleTargets.add(left);
            }
            if (0<=leftUp.x && 0<=leftUp.y) {
    			if (leftUp.x <10 && leftUp.y <10)
    				rejectedCoordinates.add(leftUp);
            }
            if (0<=leftDown.x && 0<=leftDown.y) {
    			if (leftDown.x <10 && leftDown.y <10)
    				rejectedCoordinates.add(leftDown);
            }
		...
		if ((lastTargetHit.get(i).y==lastTargetHit.get(i-1).y) && (lastTargetHit.get(i).x<lastTargetHit.get(i-1).x)) {
            ...
            Coordinates rightUpC = new Coordinates (lastTargetHit.get(i).x, lastTargetHit.get(i).y-1);
            Coordinates rightDownC = new Coordinates (lastTargetHit.get(i).x, lastTargetHit.get(i).y+1);
            Coordinates leftUpC = new Coordinates (lastTargetHit.get(i-1).x, lastTargetHit.get(i-1).y-1);
            Coordinates leftDownC = new Coordinates (lastTargetHit.get(i-1).x, lastTargetHit.get(i-1).y+1);
            ...
            if (0<=rightUpC.x && 0<=rightUpC.y) {
    			if (rightUpC.x <10 && rightUpC.y <10)
    				rejectedCoordinates.add(rightUpC);
            }
            if (0<=rightDownC.x && 0<=rightDownC.y) {
    			if (rightDownC.x <10 && rightDownC.y <10)
    				rejectedCoordinates.add(rightDownC);
            }
            
            if (0<=leftUpC.x && 0<=leftUpC.y) {
    			if (leftUpC.x <10 && leftUpC.y <10)
    				rejectedCoordinates.add(leftUpC);
            }
            if (0<=leftDownC.x && 0<=leftDownC.y) {
    			if (leftDownC.x <10 && leftDownC.y <10)
    				rejectedCoordinates.add(leftDownC);
            }
		  }
